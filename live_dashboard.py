    ...
    try:
        while run_live:
            try:
                ...
                # Safety check to prevent math crashes on empty audio buffers
                if len(audio_data) == 0:
                    continue
                ...
                alarm_prob  = (np.clip(max_scores[alarm_idx].sum(), 0, 1) * 100 * 2.5) if len(alarm_idx) else 0.0 
                ...
                # THE CRITICAL FIX: If audio drops a single frame or hiccups, ignore it and keep the mic alive!
                continue

    except Exception as e:
        status_text.error(f"Catastrophic Stream Error: {e}")
    finally:
        stream.stop_stream()
        stream.close()
        p.terminate()
